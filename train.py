...
def train(data, true_values, model,
    ...
    def train_wrapper(epoch):
        ...
        if project == 'AE' and imputation == 'VGAE':
            ...
            loss_proj = sparse_mse_loss(output_mlp,true_values)
            loss_imp = compute_loss(true_values, pred, z_mean, z_dropout, z_dispersion, gamma)
            
        if project == 'VAE' and imputation == 'GAE':
            
            output_mlp, neck1, neck2, neck3, pred = model(data)
            loss_proj = compute_loss(true_values, output_mlp, neck1, neck2, neck3, beta)
            loss_imp = lossFunc(pred, true_values)
            
        if project == 'VAE' and imputation == 'VGAE':
            
            output_mlp, neck1, neck2, neck3, pred, z_mean, z_dropout, z_dispersion = model(data)
            loss_proj = compute_loss(true_values, output_mlp, neck1, neck2, neck3, beta)
            loss_imp = compute_loss(true_values, pred, z_mean, z_dropout, z_dispersion, gamma)
